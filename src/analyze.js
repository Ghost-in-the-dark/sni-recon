...
async function verifyForward(host, port, name, refAddr, deep, opts) {
  ...
  out.checks = {
    status: sameStatus ? 'match' : 'differ (' + via.status + ' vs ' + ref.status + ')',
    bytes: sameBytes ? 'match (' + via.bytes + ')' : 'differ (' + via.bytes + ' vs ' + ref.bytes + ')',
    bodyHash: sameHash ? 'identical' : 'differ',
    leafFingerprint: sameChain === null ? 'not comparable' : sameChain ? 'identical' : 'differ'
  };
  if (!sameStatus) out.differences.push('HTTP status differs');
  if (!sameBytes) out.differences.push('body length differs by ' + Math.abs(via.bytes - ref.bytes) + ' bytes');
  if (sameBytes && !sameHash) out.differences.push('body length matches but content differs (dynamic page or injected content)');
  if (sameChain === false) out.differences.push('presented certificate is not the real site certificate');
  ...
  if (opts.assets && opts.assets.length) {
    ...
    for (const asset of opts.assets) {
      ...
      if (!entry.identical) {
        out.differences.push('asset differs from the real site: ' + asset);
...
export function scoreCandidate(c, ctx) {
  ...
  if (genuine) add(35, 'certificate chain verified offline against the public trust store');
  else add(-25, 'certificate is NOT anchored in a public trust store (lookalike identity)');

  const refFp = ctx && ctx.referenceFingerprint;
  const fpMatch = refFp && c.fingerprint256 ? refFp === c.fingerprint256 : null;
  if (fpMatch === true) add(20, 'leaf fingerprint is byte-identical to the real site');
  else if (fpMatch === false) add(-20, 'leaf fingerprint differs from the real site');
  else add(0, 'no reference fingerprint available (real hostname not resolvable from here)');

  const f = c.forward || {};
  if (f.identityMatch === true) add(25, 'proxied content is byte-identical to the real site');
  else if (f.comparable) add(10, 'proxied response matches the real site on status and size');
  else if (f.attempted && f.error) add(-10, 'could not verify the forward path through the node');
  else add(-5, 'forward path not verified');

  const st = c.stability || {};
  if (st.successRate === 1) add(5, 'handshake succeeded on every attempt');
  else if (st.successRate >= 0.8) add(1, 'handshake mostly stable (' + st.successRate + ')');
  else add(-10, 'handshake unreliable (' + st.successRate + ')');

  if (st.deterministic) add(5, 'identical certificate on every attempt');
  else add(-10, 'presented certificate varies between attempts');

  const d = c.validityDaysLeft;
  if (d != null && d > 180) add(5, 'certificate valid for another ' + d + ' days');
  else if (d != null && d > 30) add(2, 'certificate valid for another ' + d + ' days');
  else if (d != null) add(-10, 'certificate expires in ' + d + ' days');

  const refG = ctx && ctx.referenceGroups;
  const viaG = c.via && c.via.keyExchange ? c.via.keyExchange.name : null;
  if (refG && viaG) {
    if (refG === viaG) add(5, 'offers the same key exchange group as the real site (' + refG + ')');
    else add(-5, 'key exchange group differs from the real site (' + viaG + ' vs ' + refG + ')');
  }
...
export async function analyzeNode(node, options) {
  ...
  // --- Control probes -------------------------------------------------------
  emit(opts, { type: 'phase', phase: 'controls', message: 'control probes (no SNI, invalid2.invalid, random name)' });
  ...
  if (!baseline.ok && !strict.ok) {
    ...
    result.summary = {
      verdict: 'unreachable',
      message:
        'no TLS handshake completed, with or without SNI: ' +
        (baseline.errorCode || baseline.error) +
        '. The node may be down, firewalled, or not speaking TLS on this port.'
    };
    return result;
  }
  ...
  emit(opts, { type: 'phase', phase: 'discovery', message: 'probing ' + names.length + ' candidate names', total: names.length });
  ...
  if (wl.accepted.length === 0) {
    ...
    result.summary = {
      ...
      message:
        'the node completes a handshake but accepts none of the ' +
        names.length +
        ' tested names. It likely whitelists a different set; pass --server-names with the names it is configured for.',
      bestName: null
    };
    result.finishedAt = new Date().toISOString();
    return result;
  ...
  const analysed = [];
  if (opts.deep) {
    emit(opts, { type: 'phase', phase: 'deep', message: 'verifying ' + deepList.length + ' accepted names', total: deepList.length });
    ctx.deepIndex = 0;
  ...
  if (bestCandidate && bestCandidate.reference && bestCandidate.reference.address && bestCandidate.reference.ok) {
    ...
    emit(opts, { type: 'phase', phase: 'controls', message: 'checking whether the real site also serves a catch-all certificate' });
  ...
  if (opts.hoster) {
    emit(opts, { type: 'phase', phase: 'hoster', message: 'identifying the operator and checking for domain masking' });
...
function buildSummary(result) {
  ...
  if (wl.lookalikeNames.length && !wl.genuineNames.length) {
    s.notes.push('every accepted name is served with a certificate that is not publicly trusted.');
  }
  if (best && best.grade === 'poor') {
    s.notes.push('no strong cover identity found on this node.');
  }
  if (result.masking) {
    s.masking = {
    ...
    if (result.masking.masking) {
      s.notes.push(result.masking.headline);
    }
  }
  return s;
}
